...
describe('interview prep helpers', () => {
  it('preserves the full Amazon source bank and category taxonomy', () => {
    ...
    expect(INTERVIEW_QUESTIONS.filter((question) => question.sourceFamily === 'lp')).toHaveLength(139);
    expect(INTERVIEW_QUESTIONS.filter((question) => question.sourceFamily === 'functional')).toHaveLength(104);
    expect(INTERVIEW_QUESTIONS.filter((question) => question.managerOnly)).toHaveLength(38);
    ...
    const pushbackPrompt = INTERVIEW_QUESTIONS.find((question) => question.id === 'lp-customer-obsession-07');
    ...
    expect(pushbackPrompt?.followUps).toEqual(['What did you say or do in response to that request?']);
  });

  it('picks unique drill questions from the active category pool', () => {
    const categoryPool = INTERVIEW_QUESTIONS.filter((question) => question.sourceCategoryId === 'dive-deep');
    const selected = pickDrillQuestions(categoryPool, 4, ['technical_depth'], () => 0.42);

    expect(selected).toHaveLength(4);
    expect(selected.every((question) => question.sourceCategoryId === 'dive-deep')).toBe(true);
    expect(selected.every((question) => question.competency === 'technical_depth')).toBe(true);
  ...
  it('records drill ratings and updates streak across days', () => {
    ...
    expect(afterTwo.questionStats[question.id]).toEqual({ attempted: 2, solid: 1, strong: 1 });
    expect(afterTwo.competencyStats[question.competency]).toEqual({ attempted: 2, solid: 1, strong: 1 });
  ...
  it('saves, updates, and deletes STAR stories', () => {
    ...
    const saved = saveStarStory(
      initial,
      {
        ...
        situation: 'The release date was fixed and the team had major defects late in the cycle.',
        task: 'I needed to protect the launch without hiding the risk.',
        action: 'I created a red-yellow-green review, narrowed scope, and aligned engineering and support on the fallback plan.',
        result: 'We shipped on time, reduced open critical defects by 42%, and avoided customer-facing downtime.',
    ...
    const updated = saveStarStory(
      saved,
      {
        ...
        situation: 'The release date was fixed and the team had major defects late in the cycle.',
        task: 'I needed to protect the launch without hiding the risk.',
        action: 'I created a red-yellow-green review, narrowed scope, set daily exec updates, and aligned support on the fallback plan.',
        result: 'We shipped on time, reduced open critical defects by 42%, and kept churn flat that quarter.',
        reflection: 'Now I escalate risk with tighter thresholds much earlier.'
      },
      now
    );
    ...
    expect(updated.stories[0]?.categoryTags).toEqual(['think-big', 'vision-and-strategy']);
  ...
  it('sanitizes persisted progress objects during coercion', () => {
    const coerced = coerceInterviewProgress({
      ...
      masteredCardIds: [FRAMEWORK_CARDS[0].id, 42, FRAMEWORK_CARDS[0].id],
      competencyStats: {
    ...
    expect(coerced?.masteredCardIds).toEqual([FRAMEWORK_CARDS[0].id]);
    expect(coerced?.competencyStats.leadership).toEqual({ attempted: 3, solid: 0, strong: 3 });
    expect(coerced?.questionStats.a).toEqual({ attempted: 2, solid: 0, strong: 2 });
  ...
  it('scores stronger prep state higher', () => {
    ...
    progress = saveStarStory(
      progress,
      {
        ...
        action: 'I rebuilt the review process, made ownership explicit, and tracked defects daily.',
        result: 'Critical defects fell 42% and launch confidence improved.',
        reflection: 'I now use the same operating cadence on every high-risk release.'
      },
      now,
      () => 'story-1'
    );
    progress = toggleMasteredFramework(progress, FRAMEWORK_CARDS[0].id, true, now);
    progress = toggleChecklistItem(progress, GAME_DAY_CHECKLIST[0].id, true, now);

    const question = INTERVIEW_QUESTIONS.find((entry) => entry.competency === 'leadership') ?? INTERVIEW_QUESTIONS[0];
  ...
  it('tracks story category coverage across multiple Amazon tags', () => {
    ...
    const progress = saveStarStory(
      createInitialInterviewProgress(now),
      {
        ...
        action: 'I built the narrative, aligned leaders, and reset the roadmap around a shared operating plan.',
  ...
  it('grades a strong answer with a hire-level signal', () => {
    const question = INTERVIEW_QUESTIONS.find((entry) => entry.sourceCategoryId === 'think-big') ?? INTERVIEW_QUESTIONS[0];
  ...
  it('flags weak answers that lack proof and ownership', () => {
    const question = INTERVIEW_QUESTIONS.find((entry) => entry.sourceCategoryId === 'ownership') ?? INTERVIEW_QUESTIONS[0];
    const review = reviewInterviewAnswer(
      question,
    ...
    expect(review.misses.some((item) => item.toLowerCase().includes('ownership'))).toBe(true);
    expect(review.misses.some((item) => item.toLowerCase().includes('proof'))).toBe(true);
  });
  ...
  it('pressure-tests stories and surfaces upgrade paths', () => {
    const pressureTest = buildStoryPressureTest({
      ...
      action: 'I worked with the team on the migration plan and handled tasks as they came up.',
